...
class ContentRedactor:
    ...
    PATTERNS = {
        ...
        "bearer_token": (r"\bBearer\s+[A-Za-z0-9._-]+\b", "[TOKEN REDACTED]"),
    }

    # Aggressive patterns (more false positives but safer)
    AGGRESSIVE_PATTERNS = {
        "any_token": (r"\b[A-Fa-f0-9]{32,}\b", "[TOKEN REDACTED]"),
        "password_mention": (r"password\s*[:=]\s*\S+", "[PASSWORD REDACTED]"),
        "secret_mention": (r"secret\s*[:=]\s*\S+", "[SECRET REDACTED]"),
    }
...
class ContextIsolationServer:
    ...
    def process_request(
        ...
        if self.isolation_level == IsolationLevel.REQUEST:
            ...
            if session_id is None:
                ...
                if session is None:
                    ...
                    is_valid, message = self.verify_context_integrity(session_id, provided_context)
                    if not is_valid:
        ...
        if redacted_message.redactions_made:
            warnings.append(f"Redacted {len(redacted_message.redactions_made)} sensitive item(s)")
